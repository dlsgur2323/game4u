...
window.onload = function(){
	...
	<c:forEach items="${gamePictureList}" var="picture" varStatus="status">
		
		var oldPictureimg = "<%=request.getContextPath()%>/admin/getImage.do?picName=${picture.picture}"
	
		$('#picname_${status.index +1}').attr("idx","${picture.picno }");
		$('#gamePic_${status.index +1}').css({'background-image':'url('+oldPictureimg+')',
										  		'background-position':'center',
										  		'background-size':'cover',
										  		'background-repeat':'no-repeat'
											});
	</c:forEach>
	var thumbpicimg = "<%=request.getContextPath()%>/admin/getImage.do?picName=${game.thumbnail}"
	$('#gamePic_7').css({'background-image':'url('+thumbpicimg+')',
  		'background-position':'center',
  		'background-size':'cover',
  		'background-repeat':'no-repeat'
	});
	$('#picname_7').val('${game.thumbnail}');
...
function uploadPicture(obj,num){
	...
	$.ajax({
		...
		success:function(data){
			...
			$('input#picname_' + num).addClass('picChanged'); // 변경시 삭제될 파일명
